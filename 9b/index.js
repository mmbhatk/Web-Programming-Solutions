...
MongoClient.connect(url, function(err, db) {
	if(!err)
	{
		...
		app.get('/insert', function(req, res) {
			db.collection('student').insert({name: req.query.uname, branch: req.query.branch, sem: req.query.sem})
		...
		app.get('/cse6', function(req, res) {
			db.collection('student').find({branch: 'cse', sem: '6'}, {_id: 0, branch: 0, sem: 0}).toArray(function(err, data) {
				console.log(JSON.stringify(data))
				res.end(JSON.stringify(data))
			})
		})
